...
async function getPriceForFlight ({ from, to, date, number, alertType }, browser) {
...
async function getFlights ({ from, to, departDate, returnDate, browser }) {
  const twoWay = Boolean(departDate && returnDate);
  const fares = { outbound: [] };

  if (twoWay) fares.return = [];

  const html = await getPage(from, to, departDate, returnDate, browser);
...
async function getPage (from, to, departDate, returnDate, browser) {
  ...
  try {
    ...
    try {
      ...
      console.error('Unable to find flight information on page: ' + page.url());
      await page.close();
